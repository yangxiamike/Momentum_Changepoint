...
def run_module(
    ...
    Args:
        ...
        output_cpd_file_path (str): dull path, output cpd moment for specific ticker
    ...
    with open(output_cpd_file_path, 'w') as f:
        writer = csv.writer(f)
        writer.writerow(csv_fields)
    ...
    for window_end in range(lookback_window_length + 1, len(time_series_data)):
        ...
        if cp_score == 'NA':
            continue
        # write detected cpd to csv file
        if cp_score >= CPD_THRESHOLD and \
            (not cpd_result or cp_loc - cpd_result[-1] >= lookback_window_length):
            cpd_result.append(cp_loc)
            with open(output_cpd_file_path, "a") as f:
                writer = csv.writer(f)
                writer.writerow(
                    [window_date, time_index, cp_loc, cp_loc_normalised, cp_score]
                )
